...
@interface CHTumblrMenuItemButton : UIControl<UIAlertViewDelegate>
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        UITapGestureRecognizer *ges = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(touchBackground:)];
        ges.delegate = self;
        [self addGestureRecognizer:ges];
       self.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"maoboli-2.jpg"]];
...
{
    UIAlertView *alert =[[UIAlertView alloc]initWithTitle:@"提示" message:@"您还没有选择任何目录" delegate:self cancelButtonTitle:@"重选" otherButtonTitles:@"退出", nil];
    [alert show];
}
...
{
    if (buttonIndex == 1) {
        [self buttonTapped:[buttons_ lastObject]];
    }
}
